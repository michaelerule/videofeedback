 ...
 *
 * This class handles retrieval of audio data from the system, as well as
 * filtering and visualization of that data.
 *
 */
...
public final class AudioInput {
    ...
    static final AudioFormat format = new AudioFormat(SAMPLE_RATE, 8, // sampleSizeInBits
            // //8,16
            1, // channels //1,2
    ...
    float[][] harmonic;
    /**
     * writing indices for the filters
     */
    float[] index;
    /**
    ...
    public AudioInput(DoubleBuffer b, int line) {
        /*
         * audio_line = line ; ArrayList<TargetDataLine> valid_lines = new
         * ArrayList<TargetDataLine>(); try {
         * 
         * System.out.println("AUDIO MIXERS { "); Mixer.Info[] mixers =
         * AudioSystem.getMixerInfo() ; for ( Mixer.Info i : mixers ) {
         ...
         * try { m.open();
         * 
         * Line[] lines = m.getTargetLines(); if ( lines.length <= 0 )
         * System.out.println(" { no target lines }" ); else {
         * System.out.println(" TargetLines {"); for ( Line l : lines ) {
         * System.out.println( " " + l.getLineInfo() ); } System.out.println("
         * }"); }
         * 
         * lines = m.getSourceLines(); if ( lines.length <= 0 )
         * System.out.println(" { no source lines }" ); else {
         * System.out.println(" SourceLines {"); for ( Line l : lines ) {
         * System.out.println( " " + l.getLineInfo() ); } System.out.println("
         * }"); }
         * 
         * TargetDataLine.Info info = new
         * TargetDataLine.Info(DataLine.class,format); if ( m.isLineSupported(
         * info ) ) { Line t = m.getLine( info ); System.out.println(" " + t );
         * valid_lines.add((TargetDataLine)t); } m.close(); } catch( Exception e
         * ) { System.out.println("Error opening mixer"); e.printStackTrace();
         * if ( m.isOpen() ) m.close() ; } } System.out.println("}");
         * 
         * System.out.println("AUDIO LINES { "); Line.Info[] all_lines =
         * AudioSystem.getTargetLineInfo( new
         * DataLine.Info(TargetDataLine.class,format)) ; for ( Line.Info i :
         * all_lines ) { System.out.println( " " + i.getLineClass() + " {" );
         * System.out.println( " " + i.toString() ); Line l =
         * AudioSystem.getLine( i ); System.out.println( " " + l );
         * System.out.println(" }"); } System.out.println("}");
         * 
         * 
         * System.out.println("AUDIO FORMATS { "); AudioFileFormat.Type[]
         * formats = AudioSystem.getAudioFileTypes(); for ( AudioFileFormat.Type
         * i : formats ) { System.out.println( " " + i.toString() ); }
         * System.out.println("}");
         * 
         * 
         * 
         * } catch( Exception e ) { System.out.println("Error grabbing audio
         * streams"); e.printStackTrace(); }
         */
        try {

            // if ( valid_lines.size() <= 0 || line < 0 ) {
            ...
            /*
             * } else { targetDataLine =
             * valid_lines.get(audio_line%valid_lines.size());
            ...
            for (int i = 0; i < harmonic.length; i++) {
                for (int j = 0; j < harmonic.length; j++) {
                    float quot = StrictMath.abs(2 * ((index[j] / index[i]) % 1 - .5f));
                    if (quot > .9) {
                        harmonic[i][j] = quot;
                    }
                }
            ...
            buffer_size = buffer.output.data_buffer.getSize();
    ...
    public void increment_visualisation(int n) {
        ...
        int indexx = visualisations.indexOf(current_visualisation);
        if (indexx < 0) {
            current_visualisation = visualisations.get(0);
        } else {
            indexx = indexx + n;
            if (indexx < 0) {
                indexx = size - (-indexx % size);
            } else if (indexx >= size) {
                indexx %= size;
            }
            current_visualisation = visualisations.get(indexx);
        }
    }
     ...
     */
    public void render() {
        if (!(active && ((capture != null) && capture.running))) {
            return;
        }

        compute_histogram();
        current_visualisation.render(buffer.output.graphics2D);
    }

    public void simulate_tone(int i) {
        for (int j = 0; j < histogram.length; j++) {
            // float quot = (float)StrictMath.abs(2 * ((index[j] / index[i]) % 1
            // - .5f));
            // quot *= quot;
            // quot = (float)((Math.exp(quot) - 1)/(Math.E - 1));//quot*quot;
            // quot= (float)Math.exp(1 - quot) - 1;
            // data_buffer.output.data_buffer.setElem((j*image_width/harmonic.length +
            // image_width * (int)((2 - quot) * half_image_height)) %
            // buffer_size,
            // (quot > .9)? 0x00FF0000 : 0);
            // harmonic[i][j] = quot;
            ...
            // if (quot > .9)
            quot = reso;
            buffer.output.data_buffer.setElem((j * image_width / harmonic.length + image_width * (int) ((2 - quot) * half_image_height)) % buffer_size,
    ...
    void compute_histogram() {
        ...
        for (int i = 0; i < filter.length; i++) {
            ...
            float sum = 0;
            for (float aFrequency : frequency) {
                sum += aFrequency * aFrequency;
            }
        ...
        /*
         * for (int i = 0; i < filter.length; i++) { for (int j = 0; j < i; j++)
         * { histogram[j] -= harmonic[i][j] * histogram[i]; if (histogram[j] <
         * 0) histogram[j] = 0; } }
         */
        ...
        for (int i = 1; i < histogram.length / 2; i++) {
            ...
            float weightt = histogram[i] / low_volume_average;
            int RGB = Color.HSBtoRGB(i % interval / interval, 1.f, weightt);
            R += (0x000000FF & (RGB >> 16));
            G += (0x000000FF & (RGB >> 8));
            B += (0x000000FF & RGB);
        ...
        for (int i = histogram.length / 2; i < histogram.length; i++) {
            ...
            float weightt = histogram[i] / high_volume_average;
            int RGB = Color.HSBtoRGB(i % interval / interval, 1.f, weightt);
            R += (0x000000FF & (RGB >> 16));
            G += (0x000000FF & (RGB >> 8));
            B += (0x000000FF & RGB);
        ...
        spectrum_color = (((int) (R * 255) & 0x000000FF) << 16) | (((int) (G * 255) & 0x000000FF) << 8) | (((int) (B * 255) & 0x000000FF));
    ...
    VisualisationFunction piano = new VisualisationFunction() {
        ...
        public void render(Graphics output_Graphics) {
            ...
            float xscalar = (image_width - 1) / ((filter.length / SUBSAMPLE) * 7 / 12.f);
            ...
            int counter = 1;
            float color_scalar = 255 / histogram[max_pitch];// / 77.f * 256 /
            // histogram[max_pitch];;
            int px = 0;
    ...
    public static void main(String[] args) {
        ...
        Perceptron perceptron = new Perceptron("resource/Settings.txt", "resource/CrashReport.txt", "resource/presets");
        final DoubleBuffer buffer = new DoubleBuffer(perceptron, frameimage, frameimage, null, null);
