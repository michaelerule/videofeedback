...
public class Object3D {
    ...
    public static class RotateablePoint3D implements Point3D {
        ...
        public RotateablePoint3D(StringTokenizer k) {
            this(Double.parseDouble(k.nextToken()), Double.parseDouble(k.nextToken()), Double.parseDouble(k.nextToken()));
        }
    ...
    public static class InterpolatedPoint3D implements Point3D {
        ...
        public InterpolatedPoint3D(StringTokenizer k, ArrayList<Object> allpoints) {
            ArrayList<Object> newpoints = new ArrayList<>();
        ...
        public Point3D createclone(ArrayList<Object> newpoints, Point3D[] point) {// clonedrotatedpoints,
            // ArrayList
            // clonedinterpolatedpoints)
            // {
            Point3D[] newclonedpoints = new Point3D[points.length];
            for (int i = 0; i < points.length; i++) {
                for (int j = 0; j < point.length; j++) {
                    // System.out.println("\t\t"+points[i]+" :: "+point[j]);
                    if (point[j] == points[i]) {
    ...
    public class Generic3D {
        ...
        public int getComponentRGBA() {
            ...
            return (new Color((int) (s * r), (int) (s * g), (int) (s * b), a)).getRGB();
        }
        ...
        public void applyColor(Graphics G) {
            try {
                ...
                G.setColor(new Color((int) (s * r), (int) (s * g), (int) (s * b), a));
            } catch (Exception e) {
            }
        }
    }
    ...
    public class StationaryImage3D extends Generic3D implements Shape3D {
        ...
        public StationaryImage3D(StringTokenizer k, ArrayList<Object> a) {
    ...
    public class RotateableText3D extends Generic3D implements Shape3D {
        ...
        public RotateableText3D(StringTokenizer k, ArrayList<Object> a) {
            p = (Point3D) (a.get(Integer.parseInt(k.nextToken()) - 1));
            message = k.nextToken();
            if (k.nextToken() != null) {
    ...
    public class ColoredPoint3D extends Generic3D implements Shape3D {
        ...
        public ColoredPoint3D(StringTokenizer k, ArrayList<Object> a) {
            p = (Point3D) (a.get(Integer.parseInt(k.nextToken()) - 1));
            setColor(parseColor(k));
        }
        ...
        public void draw(Graphics g, BufferedImage b) {
            try {
                b.setRGB((int) p.getx(), (int) p.gety(), getComponentRGBA());
            } catch (Exception e) {
            }
        }
    }
    ...
    public class PointSphere3D extends ColoredPoint3D implements Shape3D {
        ...
        public PointSphere3D(StringTokenizer k, ArrayList<Object> a, ArrayList<Object> s) {
            super(k, a);
            ...
            for (int x = 0; x < surface; x++) {
                ...
                Point3D temp = new RotateablePoint3D(p.getx() + r * Math.cos(theta) * Math.cos(phi), p.gety() + r * Math.sin(phi), p.getz() + r
    ...
    public class Image3D extends Generic3D implements Shape3D {
        ...
        public Image3D(StringTokenizer k, ArrayList<Object> p) {
        ...
        @Override
        public Shape3D cloneto(Point3D[] oldpoints, ArrayList<Object> newpoints) {
            return new Image3D(correspondingpoint(point0, oldpoints, newpoints), correspondingpoint(point1, oldpoints, newpoints), correspondingpoint(point2,
                    oldpoints, newpoints), correspondingpoint(point3, oldpoints, newpoints), rgb);
        }
        ...
        public void draw(Graphics g, BufferedImage image_buffer) {
            ...
            double m0 = (point0.gety() - point1.gety()) / (x0 - x1), m1 = (point1.gety() - point2.gety()) / (x1 - x2), m2 = (point2.gety() - point3.gety())
                    / (x2 - x3), m3 = (point3.gety() - point0.gety()) / (x3 - x0), c0 = point0.gety() - m0 * x0, c1 = point1.gety() - m1 * x1, c2 = point2
                    .gety() - m2 * x2, c3 = point3.gety() - m3 * x3, r0 = Math.sqrt(m2 * m2 + 1) / Math.sqrt(m0 * m0 + 1), r1 = Math.sqrt(m3 * m3 + 1)
                    / Math.sqrt(m1 * m1 + 1), wr0 = w * r0, hr1 = h * r1, dy1 = (p0.gety() - p1.gety()) / (p0.getx() - p1.getx()), dy2 = (p0.gety() - p4.gety())
                    / (p0.getx() - p4.getx()), dy6 = (p3.gety() - p2.gety()) / (p3.getx() - p2.getx()), y1 = p0.gety(), y2 = y1, m0x = x * m0, m1x = x * m1, m2x = x
                    * m2, m3x = x * m3;
            ...
            if (dy1 < dy2) {
                for (; x < p1.getx() - 1; x++) {
                    for (int y = (int) y1; y <= y2; y++) {
                        try {
                            image_buffer.setRGB(x, y, rgb[(int) (wr0 / (r0 - (m2x - y + c2) / (m0x - y + c0)))][(int) (hr1 / (r1 - (m3x - y + c3)
                                    / (m1x - y + c1)))]);
                        } catch (Exception e) {
                        }
                    }
    ...
    public class Circle3D extends Generic3D implements Shape3D {
        ...
        public Circle3D(StringTokenizer k, ArrayList<Object> p) {
    ...
    public class Sphere3D extends Circle3D {
        ...
        public Sphere3D(StringTokenizer k, ArrayList<Object> p) {
            super(k, p);
        }
    ...
    public class Line3D extends Generic3D implements Shape3D {
        ...
        public Line3D(StringTokenizer k, ArrayList<Object> p) {
            a = (Point3D) (p.get(Integer.parseInt(k.nextToken()) - 1));
        ...
        @Override
        public Shape3D cloneto(Point3D[] oldpoints, ArrayList<Object> newpoints) {
            return new Line3D(correspondingpoint(a, oldpoints, newpoints), correspondingpoint(b, oldpoints, newpoints), getColor());
        }
    ...
    public class ThickLine3D extends Line3D {
        ...
        public ThickLine3D(StringTokenizer k, ArrayList<Object> p) {
            a = (Point3D) (p.get(Integer.parseInt(k.nextToken()) - 1));
            b = (Point3D) (p.get(Integer.parseInt(k.nextToken()) - 1));
    ...
    public class ShadedLine3D extends ThickLine3D {
        ...
        private int[][] rgba;
        private final int color_shading = -100;
        ...
        public ShadedLine3D(StringTokenizer k, ArrayList<Object> p) {
            super(k, p);
            if (k.hasMoreTokens()) {
                k.nextToken();
                setColor(parseColor(k));
        ...
        public void applyColor(Graphics G, int i) {
            ...
            G.setColor(new Color((int) (Math.max(0, s) * rgba[i][0]), (int) (Math.max(0, s) * rgba[i][1]), (int) (Math.max(0, s) * rgba[i][2]), rgba[i][3]));
        }
    ...
    public class Outline3D extends Generic3D implements Shape3D {
        ...
        public Outline3D(StringTokenizer k, ArrayList<Object> l) {
    ...
    public class Form3D extends Outline3D {
        ...
        public Form3D(StringTokenizer k, ArrayList<Object> l) {
            super(k, l);
        }
    ...
    public class Cylinder3D extends Line3D {
        ...
        public Cylinder3D(StringTokenizer k, ArrayList<Object> p) {
            a = (Point3D) (p.get(Integer.parseInt(k.nextToken()) - 1));
            b = (Point3D) (p.get(Integer.parseInt(k.nextToken()) - 1));
        ...
        public void draw(Graphics g, BufferedImage image_buffer) {
            ...
            while (i < end) {
                int index1 = i % perimeter.length, index2 = j % perimeter.length;
    ...
    public class TaperedStem3D extends Cylinder3D {
        ...
        public TaperedStem3D(StringTokenizer k, ArrayList<Object> p) {
            a = (Point3D) (p.get(Integer.parseInt(k.nextToken()) - 1));
            b = (Point3D) (p.get(Integer.parseInt(k.nextToken()) - 1));
            radius = Double.parseDouble(k.nextToken());
        ...
        public void draw(Graphics g, BufferedImage image_buffer) {
            ...
            int index1 = i1 % perimeter.length, index2 = (i2 + perimeter2.length) % perimeter2.length;
            g.drawLine((int) (perimeter[index1][0] + a.getx()), (int) (perimeter[index1][1] + a.gety()), (int) (perimeter2[index2][0] + b.getx()),
    ...
    public class HollowCylinder3D extends Cylinder3D {
        ...
        public HollowCylinder3D(StringTokenizer k, ArrayList<Object> p) {
            super(k, p);
        }
        ...
        public void draw(Graphics g, BufferedImage image_buffer) {
            ...
            int index1 = i % perimeter.length, index2 = j % perimeter.length;
            g.drawLine((int) (perimeter[index1][0] + a.getx()), (int) (perimeter[index1][1] + a.gety()), (int) (perimeter[index2][0] + b.getx()),
    ...
    public class SparseCylinder3D extends Cylinder3D {
        ...
        public SparseCylinder3D(StringTokenizer k, ArrayList<Object> p) {
            super(k, p);
        }
    ...
    public class Cone3D extends Cylinder3D {
        ...
        public Cone3D(StringTokenizer k, ArrayList<Object> a) {
            super(k, a);
        }
        ...
        public void draw(Graphics g, BufferedImage image_buffer) {
            ...
            int end = i + perimeter.length / 2;
            while (i < end) {
                int index1 = i % perimeter.length, p1 = (int) (perimeter[index1][0] + a.getx());
                p2 = (int) (perimeter[index1][1] + a.gety());
                p3 = (int) (b.getx());
                p4 = (int) (b.gety());
                g.drawLine(p1, p2, p3, p4);
    ...
    public class ShadedCylinder3D extends Cylinder3D {
        ...
        public ShadedCylinder3D(StringTokenizer k, ArrayList<Object> p) {
            a = (Point3D) (p.get(Integer.parseInt(k.nextToken()) - 1));
            b = (Point3D) (p.get(Integer.parseInt(k.nextToken()) - 1));
            radius = Double.parseDouble(k.nextToken());
            if (k.hasMoreTokens()) {
        ...
        public void draw(Graphics g, BufferedImage image_buffer) {
            ...
            while (i < end) {
                ...
                int index1 = i % perimeter.length, index2 = j % perimeter.length;
                g.drawLine(p1 = (int) (perimeter[index1][0] + a.getx()), p2 = (int) (perimeter[index1][1] + a.gety()),
                        p3 = (int) (perimeter[index2][0] + b.getx()), p4 = (int) (perimeter[index2][1] + b.gety()));
                i++;
                j--;
                c = (c + 1) % rgba.length;
            }
    ...
    public Color parseColor(StringTokenizer k) {
        return new Color((k.hasMoreTokens()) ? Integer.parseInt(k.nextToken()) : 127, (k.hasMoreTokens()) ? Integer.parseInt(k.nextToken()) : 127,
                (k.hasMoreTokens()) ? Integer.parseInt(k.nextToken()) : 127, (k.hasMoreTokens()) ? Integer.parseInt(k.nextToken()) : 255);
    }

    Point3D[] point; // all points, ever
    RotateablePoint3D[] p_rotateable; // rotateable points, clone of
    // m_p_rotateable
    ...
    public Object3D(BufferedReader b) {
        ...
        ArrayList<Object> p = new ArrayList<>(), s = new ArrayList<>(), pr = new ArrayList<>(), prm = new ArrayList<>(), pi = new ArrayList<>();
    ...
    public Object3D smooth() {
        ...
        ArrayList<Object> outlines = new ArrayList<>(), // will store existing outlines
                newshapes = new ArrayList<>(), // will store new outlines
                newpoints = new ArrayList<>(), // will store clones of all points and new points
                allnewshapes = new ArrayList<>(), newrotateablepoints = new ArrayList<>(), // will  store clones of existing rotateable points
        ...
        for (RotateablePoint3D aM_p_rotateable : m_p_rotateable) {
            newmasterrotatreablepoints.add(aM_p_rotateable.clone2());
        }

        for (RotateablePoint3D aP_rotateable : p_rotateable) {
            RotateablePoint3D p = (RotateablePoint3D) (aP_rotateable.clone2());
            newrotateablepoints.add(p);
            newpoints.add(p);
        }
        // clones existing interpolated points
        for (InterpolatedPoint3D aP_interpolated : p_interpolated) {
            InterpolatedPoint3D p = (InterpolatedPoint3D) (aP_interpolated.createclone(newpoints, point));
            newinterpolatedpoints.add(p);
            newpoints.add(p);
        }
        // stores lines as points in an arraylist, one arraylist for each point
        ArrayList<Point3D>[] lines = new ArrayList[point.length];
        // stores midpoints of the lines in ArrayList lines
        ArrayList<Point3D>[] linemidpoints = new ArrayList[point.length];
        ...
        for (Shape3D aShape : shape) {
            try {
                // if an outlines ins found:
                Outline3D o = (Outline3D) aShape;
                outlines.add(o);
                ...
                // where the other points linking to cloned previous should be placed in the lines[] hopefully the index
                // of the cloned previous in line is the same as the index of uncloned previous in points
                ...
                for (int j = 0; j < o.p.length; j++) {
                    ...
                    if (newpoints.get(previousindex) != cloned) {
                        ...
                        // where the other points linking to cloned should be
                        // placed int the lines[]
                        // hopefully the index of the clone in lines[] is the
                        // same as the index of uncloned in points
                        ...
                        if (lines[previousindex].contains(cloned)) {
                            midpoint = (InterpolatedPoint3D) (linemidpoints[previousindex].get(lines[previousindex].indexOf(cloned)));
                        } else {
            ...
            } catch (Exception e) {// the shape is not a valid outline or form 3d
                // e.printStackTrace();
                try {
                    allnewshapes.add(aShape.cloneto(point, newpoints));
                } catch (Exception e1) {
                }
            }
        }
        ...
        // now to locate and create ne faces/outlines created from
        // interpolations
        for (int i = 0; i < point.length; i++) {
            // investigate every point that has at least three lines
            if (lines[i].size() > 2) {
                // now comes the tricky part, correctly reconstructing an outline

                // store all the midpoints (to become points of an outline) an a
                // new array
                Point3D[] p = new Point3D[linemidpoints[i].size()];
                for (int j = 0; j < linemidpoints[i].size(); j++) {
                    p[j] = linemidpoints[i].get(j);
                }

                // create a new array that will hold the outline points in
                // correct order
        ...
        return new Object3D(newpoints, newrotateablepoints, newmasterrotatreablepoints, newinterpolatedpoints, allnewshapes);
    }

    private Object3D(ArrayList<Object> p, ArrayList<Object> pr, ArrayList<Object> prm, ArrayList<Object> pi, ArrayList<Object> s) {
        finalize(p, pr, prm, pi, s);
    }
    ...
    void finalize(ArrayList<Object> p, ArrayList<Object> pr, ArrayList<Object> prm, ArrayList<Object> pi, ArrayList<Object> s) {
    ...
    boolean parse(String t, StringTokenizer k, ArrayList<Object> shape_destination, ArrayList<Object> point_destination,
                  ArrayList<Object> master_rotateable_point_destination, ArrayList<Object> rotateable_point_destination,
                  ArrayList<Object> interpolated_point_destination) {
        ...
        if (t.equals("r") || t.equals("vr") || t.equals("interpolatedpoint") || t.equals("intepoint")) {
            ...
            try {
                RotateablePoint3D p = new RotateablePoint3D(Double.parseDouble(t), Double.parseDouble(k.nextToken()), Double.parseDouble(k.nextToken()));
                point_destination.add(point_counter, p);
                rotateable_point_destination.add(p);
                master_rotateable_point_destination.add(p.clone2());
                point_counter++;
    ...
    public void rotatex(double t) {
        ...
        double temp;
        for (RotateablePoint3D aM_p_rotateable : m_p_rotateable) {
            temp = aM_p_rotateable.y;
            aM_p_rotateable.y = c * aM_p_rotateable.y + s * aM_p_rotateable.z;
            aM_p_rotateable.z = c * aM_p_rotateable.z - s * temp;
        }
    }
    ...
    public void rotatey(double t) {
        ...
        double temp;
        for (RotateablePoint3D aM_p_rotateable : m_p_rotateable) {
            temp = aM_p_rotateable.x;
            aM_p_rotateable.x = c * aM_p_rotateable.x + s * aM_p_rotateable.z;
            aM_p_rotateable.z = c * aM_p_rotateable.z - s * temp;
        }
    }
    ...
    public void rotatez(double t) {
        ...
        for (RotateablePoint3D aM_p_rotateable : m_p_rotateable) {
            temp = aM_p_rotateable.x;
            aM_p_rotateable.x = c * aM_p_rotateable.x + s * aM_p_rotateable.y;
            aM_p_rotateable.y = c * aM_p_rotateable.y - s * temp;
        }
    }

    public void shiftx(double dx) {
        for (RotateablePoint3D aM_p_rotateable : m_p_rotateable) {
            aM_p_rotateable.x += dx;
        }
    }

    public void shifty(double dy) {
        for (RotateablePoint3D aM_p_rotateable : m_p_rotateable) {
            aM_p_rotateable.y += dy;
        }
    }

    public void shiftz(double dz) {
        for (RotateablePoint3D aM_p_rotateable : m_p_rotateable) {
            aM_p_rotateable.z += dz;
        }
    }
    ...
    private static Point3D correspondingpoint(Point3D query, Point3D[] oldpoints, ArrayList<Object> newpoints) {
        for (int i = 0; i < oldpoints.length; i++) {
            if (query == oldpoints[i]) {
                return (Point3D) (newpoints.get(i));
            }
        }
        return null;
    }
    ...
    public Object3D merge(Object3D o) {
        ...
        ArrayList<Object> prm = new ArrayList<>();
        ArrayList<Object> pi = new ArrayList<>();
        ArrayList<Object> s = new ArrayList<>();

        Collections.addAll(prm, m_p_rotateable);
        Collections.addAll(pi, p_interpolated);
        Collections.addAll(pr, p_rotateable);
        Collections.addAll(p, point);
        Collections.addAll(s, shape);
        Collections.addAll(prm, o.m_p_rotateable);
        Collections.addAll(pi, o.p_interpolated);
        Collections.addAll(pr, o.p_rotateable);
        Collections.addAll(p, o.point);
        Collections.addAll(s, o.shape);

        return new Object3D(p, pr, prm, pi, s);
    }
}
