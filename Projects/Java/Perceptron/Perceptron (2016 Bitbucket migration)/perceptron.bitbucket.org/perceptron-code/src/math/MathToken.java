 ...
 *
 *          <p/>This class describes a single object that may be encountered in a
 *          mathematical equation, which may be a constant, a variable, a function, or
 *          an operation on the numbers. This class contains static methods that convert
 *          a user inputted Equation into an instance of Equation, a special binary tree
 *          of MathTokens that allows the computer to evaluate custom equations. The
 *          conversion methods are not efficient, but they are not used excessively
 *          either. This class also contains methods for performing any possible
 *          operation stored in a MathToken, with reasonable efficiency.</p>
 *
...
public class MathToken {
    ...
    // Returns true if all data fields in this MathTOken match those in
    // otherToken
    public boolean equals(MathToken otherToken) {
        return (otherToken.getToken() == myToken && otherToken.getVar() == myVar && otherToken.getNumber() == myNumber);
    }
    ...
    /**
     * *************************************************************************
     * The Following Methods are all static and are used inconverting a String
     * into an Equation.
     * <p/>
     * *********************************************************************
     */
    ...
    public static MathToken extractToken(String data) {
        ...
        try {
            ...
            j = 0;
            while (j < masterTokens.length) {
                k = 0;
                while (k < masterTokens[j].length) {
    ...
    /**
     * converts an array of math tokens into a branched form based on the order
     * of operations
     */
    ...
    /**
     * parses a string into a usable array of math tokens
     */
    private static MathToken[] toTokenArray(String data) {
        ...
        while (data.length() > 0 && i < token.length) {
            while (data.charAt(0) > 255) {
                data = data.substring(1); // if it finds a non ascii character it pulls it off and ignores it
            }
            float num = 0;
            j = 1;
            while (j <= data.length()) {
    ...
    // scrubs tokens, placing any implied * and turning any lone - signs into
    // signs
    private static MathToken[] scrub(MathToken[] token) {
        if (token.length > 0) {
            ...
            int i = 1;
            if (token[0].equals(2)) {/*
                 * special case for the first token, if its
                 * a - sign make it a _ sign
                 */
                if (token[1].equals(-2)) {/*
                     * if its followed by a number, make
                     * that number negetive
                     */
            ...
            while (i < token.length) {
                if (token[i] != null) {
                    ...
                    if ((b || token[i].equals(7)) && (!token[i - 1].isOperator()) || b && token[i - 1].equals(8)) {// insert
                        // multiplication
                        // sign
                    ...
                    // {/*If it is not at the last token (or the last token is a
                    // !,),#,v*/
                    if (token[i].equals(2) && token[i - 1].isOperator()) {/*
                         * if it finds a - sign following another operator, it investigates for:   */
                        if (token[i + 1].equals(-2)) {
                            /*
                             * if it found a -sign in front of a number that is
                             * definitely not an subtraction, then it makes the
                             * number negative
                             */
                         ...
                         * otherwise, it turns the - sign into a _()
                         * function, which will negate the contents of
                         * the following expression or variable
                         */ {
    ...
    /*
     * replaces indices start through end-1 with the new MathToken[]
     */
    ...
    /*
     * deletes indices start through end-1
     */
    ...
    /*
     * copies indices start through end-1
     */
    ...
    /*
     * finds the index of the close parenthesis corresponding to the open
     * parenthesis at the passed index, used to break up an array of MathTokens
     * into a group of smaller, linked arrays, an intermediate step to making an
     * Equation
     */
    ...
    // Overrides toString in Object, converts the data of this MathToken to
    // String format
    @Override
    ...
    /**
     * *************************************************************************
     * ** Remaining methods are associated with the main block, which tests the
     * functionality of this class *
     * ***************************************************************************
     */
    /*
     * This method returns a String of all legal functions in MathToken, used by
     * the main block testing method
     */
    ...
    /*
     * The main block acts as a calculator to test the math operations
     * contained in MathToken (see the welcome message for details)
     */
    public static void main(String args[]) throws IOException {
        System.out.println("\n___________________________________________________________________________________\n"
                ...
                + " to display a list of legal functions and operators.\n"
                + "___________________________________________________________________________________\n");
    ...
    public final static String[][] masterTokens = {
            ...
            */ {"rpart", "realpart", "real"},
            /*
            * 15
            */ {"ipart", "imagpart", "imag", "imagenary", "imagenarypart"},
            /*
            * 16
            */ {"abs", "absolute", "absolutevalue"},
            /*
            * 17
            */ {"int", "integer", "intof", "integerof", "integerpart", "integerpartof"},
            /*
            * 18
            */ {"sign", "signum", "sgn"},
            /*
            * 19
            */ {"arg", "Complexarg", "argument", "Complexargument"},
            /*
            * 20
            */ {"conj", "conjugate", "Complexconjugate", "Complexconj"},
            /*
            * 21
            */ {"sin", "sine"},
            /*
            ...
            */ {"cot", "cotangent", "cotan"},
            /*
            * 27
            */ {"asin", "arcsin", "asine", "arcsine", "inversesin", "inversesine"},
            /*
            * 28
            */ {"acos", "arccos", "acosine", "arccosine", "acosin", "arccosin", "arcosine", "arcosin", "arcos", "inversecos", "inversecosine", "inversecosin"},
            /*
            * 29
            */ {"atan", "arctan", "atangent", "arctangent", "inversetangent", "inversetan"},
            /*
            * 30
            */ {"acsc", "acosecant", "acsec", "acosec", "arcsc", "arcosecant", "arcosec", "arccsc", "arccosecant", "arccsec", "arccosec", "inversecsc",
            "inversecosecant", "inversecsec", "inversecosec"},
            /*
            * 31
            */ {"asec", "asecant", "arcsec", "arcsecant", "inversesec", "inversesecant"},
            /*
            * 32
            */ {"acot", "acotangent", "acotan", "arccot", "arccotangent", "arccotan", "inversecot", "inversecotangent", "inversecotan"},
            /*
            * 33
            */ {"sinh", "sh", "sineh", "hyperbolicsine", "hyperbolicsin", "hsin", "hsine"},
            /*
            * 34
            */ {"cosh", "cosineh", "cosinh", "hyperboliccos", "hcos", "hcosin", "hcosine", "hyperboliccosine", "hyperboliccosin", "hyperbolicos",
            "hyperbolicosine", "hyperbolicosin"},
            /*
            * 35
            */ {"tanh", "hyperbolictangent", "htan", "hyperbolictan", "htangent", "tangenth"},
            /*
            * 36
            */ {"csch", "hyperbolicsc", "hyperbolicosecant", "hyperbolicosec", "hyperboliccsc", "hyperboliccosecant", "hyperboliccsec", "hyperboliccosec",
            "hcsc", "hcosecant", "hcsec", "hcosec", "cosecanth", "csech", "cosech"},
            /*
            * 37
            */ {"sech", "hyperbolicsec", "hyperbolicsecant", "hsec", "hsecant", "secanth"},
            /*
            * 38
            */ {"coth", "hyperbolicot", "hyperbolicotangent", "hyperbolicotan", "hyperboliccot", "hyperboliccotangent", "hyperboliccotan", "hcot",
            "hcotangent", "hcotan", "cotangenth", "cotanh"},
            /*
            * 39
            */ {"asinh", "asineh", "ahsin", "archsin", "ahsine", "archsine", "arcsinh", "arcsineh", "ahyperbolicsine", "ahyperbolicsin", "archyperbolicsine",
            "archyperbolicsin", "inversesinh", "inversesineh", "inversehyperbolicsine", "inversehyperbolicsin", "inversehsin", "inversehsine"},
            /*
            * 40
            */ {"acosh", "arcosh", "arcosineh", "arcosinh", "arccosh", "arccosineh", "arccosinh", "archyperboliccos", "archcos", "archcosin", "archcosine",
            "archyperboliccosine", "archyperboliccosin", "archyperbolicos", "archyperbolicosine", "archyperbolicosin", "acosineh", "acosinh",
            "ahyperboliccos", "ahcos", "ahcosin", "ahcosine", "ahyperboliccosine", "ahyperboliccosin", "ahyperbolicos", "ahyperbolicosine",
            "ahyperbolicosin", "inversecosh", "inversecosineh", "inversecosinh", "inversehyperboliccos", "inversehcos", "inversehcosin",
            "inversehcosine", "inversehyperboliccosine", "inversehyperboliccosin", "inversehyperbolicos", "inversehyperbolicosine",
            "inversehyperbolicosin"},
            /*
            * 41
            */ {"atanh", "ahyperbolictangent", "ahtan", "ahyperbolictan", "ahtangent", "atangenth", "arctanh", "archyperbolictangent", "archtan",
            "archyperbolictan", "archtangent", "arctangenth", "inversetanh", "inversehyperbolictangent", "inversehtan", "inversehyperbolictan",
            "inversehtangent", "inversetangenth"},
            /*
            * 42
            */ {"acsch", "ahyperbolicsc", "ahyperbolicosecant", "ahyperbolicosec", "ahyperboliccsc", "ahyperboliccosecant", "ahyperboliccsec",
            "ahyperboliccosec", "ahcsc", "ahcosecant", "ahcsec", "ahcosec", "acosecanth", "acsech", "acosech", "arcsch", "arcosecanth", "arcosech",
            "archyperbolicsc", "archyperbolicosecant", "archyperbolicosec", "archyperboliccsc", "archyperboliccosecant", "archyperboliccsec",
            "archyperboliccosec", "archcsc", "archcosecant", "archcsec", "archcosec", "arccsch", "arccosecanth", "arccsech", "arccosech",
            "inversehyperbolicsc", "inversehyperbolicosecant", "inversehyperbolicosec", "inversehyperboliccsc", "inversehyperboliccosecant",
            "inversehyperboliccsec", "inversehyperboliccosec", "inversehcsc", "inversehcosecant", "inversehcsec", "inversehcosec", "inversecsch",
            "inversecosecanth", "inversecsech", "inversecosech"},
            /*
            * 43
            */ {"asech", "ahyperbolicsec", "ahyperbolicsecant", "ahsec", "ahsecant", "asecanth", "archyperbolicsec", "archyperbolicsecant", "archsec",
            "archsecant", "arcsech", "arcsecanth", "inversehyperbolicsec", "inversehyperbolicsecant", "inversehsec", "inversehsecant", "inversesech",
            "inversesecanth"},
            /*
            * 44
            */ {"acoth", "ahyperbolicot", "ahyperbolicotangent", "ahyperbolcolicotan", "ahyperboliccot", "ahyperboliccotangent", "ahyperboliccotan", "ahcot",
            "ahcotangent", "ahcotan", "acotangenth", "acotanh", "arcoth", "arcotangenth", "arcotanh", "archyperbolicot", "archyperbolicotangent",
            "archyperbolicotan", "archyperboliccot", "archyperboliccotangent", "archyperboliccotan", "archcot", "archcotangent", "archcotan",
            "arccoth", "arccotangenth", "arccotanh", "inversehyperbolicot", "inversehyperbolicotangent", "inversehyperbolicotan",
            "inversehyperboliccot", "inversehyperboliccotangent", "inversehyperboliccotan", "inversehcot", "inversehcotangent", "inversehcotan",
            "inversecoth", "inversecotangenth", "inversecotanh"},
            /*
            * 45
            */ {"zeta", "reimannzeta"},
            /*
