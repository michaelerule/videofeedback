...
public class complex {
    ...
    final public static float two_pi = 2 * pi;
    // Common complex constants
    final public static complex I = new complex(0, 1);
    final public static complex E = new complex(e);
    final public static complex PI = new complex(pi);
    final public static complex PHI = new complex((float) ((Math.sqrt(5) - 1) / 2));
    ...
    public static complex sqrt(complex num) {
        return fromPolar((float) Math.pow(num.rSquared(), .25), arg(num) * .5f);
    }
    ...
    public static complex asin(complex num) {
        ...
        return new complex((float) Math.asin((c - d) * .5f), (float) Math.log(((float) Math.sqrt(2 * (d * c + g - 2)) + c + d) * .5f) * sign2(b));
    }
    ...
    public static complex acos(complex num) {
        ...
        c = (float) Math.sqrt(g + a);
        d = (float) Math.sqrt(g - a);
        return new complex((float) Math.acos((c - d) * .5f), -1 * (float) Math.log(((float) Math.sqrt(2 * (d * c + g - 2)) + c + d) * .5f) * sign2(b));
    }
    ...
    public static complex sinh(complex num) {
        return ((E.toThe(num)).minus(E.toThe(num.scale(-1)))).over(new complex(2));
    }

    public static complex cosh(complex num) {
        return ((E.toThe(num)).plus(E.toThe(num.scale(-1)))).over(new complex(2));
    }
    ...
    public static complex asech(complex num) {
        complex temp = oneOver(num);
        complex temp2 = new complex(1);
        return ln((sqrt(temp.minus(temp2)).times(sqrt(temp2.plus(temp)))).plus(temp));
    }
    ...
    public static complex abs(complex num) {
        if (num.imag == 0) {
            return new complex(Math.abs(num.real));
        }
    ...
    public static complex round(complex num) {
        return new complex(Math.round(num.real), Math.round((int) num.imag));
    }
    ...
    public static float tanh(float n) {
        return ((float) Math.exp(2 * n) - 1) / ((float) Math.exp(2 * n) + 1);
    }
    ...
    public static complex eToThe(complex num) {
        return fromPolar((float) Math.exp(num.real), num.imag);
    }
    ...
    public static complex ln(complex num) {
        return new complex((float) Math.log(mod(num)), arg(num));
    }
    ...
    /**
     * Initializes a new complex number with real part Real and imagenary part
     * Imag
     */
    ...
    /**
     * this returns the complex modulus, the distance between complex number
     * real,imag and the origin
     */
    ...
    /**
     * this returns the angle made between the +real axis and the line drawn
     * from the complex number real,imag to the origin A.k.a. the atan2
     * implementation similar to Ultra Fractal.
     */
    ...
    /**
     * This returns the i value squared plus the r value squared, a very common
     * calculation
     */
    ...
    public complex devidedBy(complex n) {
        ...
        return new complex((real * n.real + imag * n.imag) * num, -((real * n.imag - imag * n.real) * num));
    }
    ...
    public complex toThe(complex n) {
        ...
        return fromPolar((float) Math.pow(intrCalc1, expReal * .5f) * (float) Math.exp(-expImag * baseArg),
                expReal * baseArg + .5f * expImag * (float) Math.log(intrCalc1));
    }
    ...
    public complex toThe(float n) {
        return fromPolar((float) Math.pow(real * real + imag * imag, n * .5f), n * arg(this));
    }
    ...
    public static complex zeta(complex z) {
        ...
        for (complex k = new complex(1); k.real < 40; k.real++) {
            ...
            r += K.real;
            i += K.imag;
            if (Float.isNaN(r) || Float.isInfinite(r) || Float.isNaN(i) || Float.isInfinite(i)) {
                break;
            }
        }
