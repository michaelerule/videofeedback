...
public class AudioInput {
    ...
    public AudioInput(DoubleBuffer b, int line) {
        ...
        try {
            ...
            /*} else {
            targetDataLine = valid_lines.get(audio_line%valid_lines.size());
    ...
    public void increment_visualisation(int n) {
        ...
        int ix = visualisations.indexOf(current_visualisation);
        if (ix < 0) {
            ...
            current_visualisation = visualisations.get(ix);
        }
    }
    ...
    final void update_weights() {
        w2 = weight * volume;
        w1 = 1 - weight;
    }

    /** Turns this AudioInput on or of
     * @param a
     */
    ...
    public void render() {
        ...
        compute_histogram();
        current_visualisation.render(buffer.out.g);
    }
    ...
    public void simulate_tone(int i) {
        for (int j = 0; j < histogram.length; j++) {
            ...
            //if (quot > .9)
            quot = reso;
            buffer.out.buf.setElem((j * image_width / harmonic.length + image_width * (int) ((2 - quot) * half_image_height)) % buffer_size,
    ...
    void compute_histogram() {
        ...
        for (int i = 1; i < histogram.length / 2; i++) {
            ...
            float weight_ = histogram[i] / low_volume_average;
        ...
        for (int i = histogram.length / 2; i < histogram.length; i++) {
            ...
            float weight_ = histogram[i] / high_volume_average;
            int RGB = Color.HSBtoRGB(i % interval / interval, 1.f, weight_);
            R += (0x000000FF & (RGB >> 16));
            G += (0x000000FF & (RGB >> 8));
            B += (0x000000FF & RGB);
