...
public class MathToken {
    ...
    private static float toFloat(String data) throws NumberFormatException {
        double result = 0;
        int i,j;
        if (data.charAt(0) == '-') j = i = 2;
        else j = i = 1;
        for (; i <= data.length(); i++) {
            try {result = Double.parseDouble(data.substring(0,i));} catch (NumberFormatException e) {break;}}
        if (i == j) throw new NumberFormatException("no number here");
        return (float)result;
    }
    ...
    public static MathToken extractToken(String data) {
        int j,k;
        try {return new MathToken(toFloat(data));} catch (Exception e) {
            boolean done = false;
            j = 0; while (j < masterTokens.length && !done){
                k = 0; while ( k < masterTokens[j].length && !done) {
    ...
    private static MathToken [] toTokenArray(String data) {
        ...
        while (data.length()>0 && i < token.length) {
            while ((int)data.charAt(0) > 255) data = data.substring(1); //if it finds a non ascii character it pulls if off and ignores it
            float num = 0;
            j = 1;
            while (j<=data.length()) {
                try {
                    if (data.charAt(j-1) != '.') {
                        num = (float)(Double.parseDouble(data.substring(0,j)));}} catch (NumberFormatException e) {break;}
                j++;}
            if (j != 1) {
                data = data.substring(j-1);
                token[i] = new MathToken(num);} else {//if it wasn't a number, figure out what it was by scanning masterTokens
                boolean done = false;
                int tokenLength = 0;
                j = 0;
                ...
                if (token[i] == null && data.length()>0) {
                    ...
                    data = data.substring(1);} else data = data.substring(tokenLength);
                }
            i++;}
        return scrub(token);
    }
        ...
        if (token.length>0) {
            ...
            if (token[0].equals(2)) {/*special case for the first token, if its a - sign make it a _ sign*/
                if (token[1].equals(-2)) {/*if its followed by a number, make that number negetive*/
                    ...
                    i++;
                } else scrubber[0] = new MathToken(11);
                ...
                if (token[i] != null) {
                    ...
                    if (token[i].equals(2)&&token[i-1].isOperator()) {/*if it finds a - sign following another operator, it investigates for:*/
                        if (token[i+1].equals(-2)) {
                            ...
                            i++;} else if (token[i-1].equals(8)  || token[i].equals(9)) {
                            scrubber[placePosition] = new MathToken(3);
                            i--;} else /*otherwise, it turns the - sign into a _() function, which will negate the contents of the following expression or variable*/
                                scrubber[placePosition] = new MathToken(11);
                    } else scrubber[placePosition] = token[i];
                    placePosition++;
                }
                i++;
            }
...
into a group of smaller, linked arrays, an intermediate step to making an Equation*/
    ...
    public static int getEndParenthesis(MathToken [] token, int start) {
        ...
        while (parenthesis > 0 && i < token.length) {
            if (token[i].equals(7)) parenthesis++;
            else if (token[i].equals(8)) parenthesis--;
            i++;
        }
        return i;
    }
...
//Overrides toString in Object, converts the data of this MathToken to String format
    ...
    public String toString() {
        if (myToken == 0) return "";
        if (myToken == -1) return ""+myVar;
        if (myToken == -2) return ""+myNumber;
