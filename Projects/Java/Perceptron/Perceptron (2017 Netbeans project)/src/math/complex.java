 ...
 *  <p>
 *  This is a reasonably well featured complex number implementation. Several
 *  complex versions of transcendental functions are also implemented here.
 *
...
public class complex {
    ...
    public static complex atan(complex num) {
        float b = num.imag;
        float a = num.real;
        //if (b == 0) return new complex((float)Math.atan(a));
        float c = a*a;
    ...
    public static complex asec(complex num) {
        return (new complex(pi/2)).plus(I.times(ln((sqrt((new complex(1)).
          minus(oneOver(num.squared())))).plus(I.over(num)))));}
    ...
    public static complex csch(complex num) {
        return (new complex(2)).over((E.toThe(num)).
          minus(E.toThe(num.scale(-1))));}
    ...
    public static complex abs(complex num) {
        if (num.imag == 0) return new complex((float)Math.abs(num.real));
    ...
    public static complex parseComplex(String data) {
        ...
        String iPart;
        if (data.indexOf('+',1) > 0) iPart = data.substring(
          breakIndex+1,data.length());
        else iPart = data.substring(breakIndex,data.length());
    ...
    public complex(String data) {
        ...
        if (breakIndex < 0) breakIndex = data.indexOf('-',1);
        String iPart;
        if (data.indexOf('+',1) > 0) iPart = data.substring(breakIndex+1,data.length());
        else iPart = data.substring(breakIndex,data.length());
    ...
    public String toString() {
        ...
        if (real == 0) {
            ...
            if (imag == 1) return new String(toSTring(real)+"+"+"i");
            return new String(toSTring(real)+"+"+toSTring(imag)+"i");}
        else if (imag == 0) return new String(""+toSTring(real));
        else if (imag < 0) {
            if (imag == 1) return new String(toSTring(real)+"+"+"i");
            return new String(toSTring(real)+toSTring(imag)+"i");}
        return "NaN";}
     ...
     * @return
     */
    public float rSquared() {return real*real+imag*imag;}
    ...
    public static complex zeta( complex z ) {
        ...
        for ( complex k = new complex(1) ; k.real < 40 ; k.real ++ ) {
            ...
            if ( Float.isNaN(r) || Float.isInfinite(r) || 
                    Float.isNaN(i) || Float.isInfinite(i) ) break ;
        }
