...
public class Object3D {
    ...
    public class StationaryImage3D extends Generic3D implements Shape3D
	{
                ...
                public StationaryImage3D(StringTokenizer k, ArrayList a)
		{
			...
			image = (new ImageIcon(Object3D.class.getResource(k.nextToken()))).getImage();
		}
                ...
                public int setDepth()
		{
			...
			return new StationaryImage3D(correspondingpoint(p,oldpoints,newpoints),image);
		}
        ...
        public class Circle3D extends Generic3D implements Shape3D
	{
		...
		{
			point = (Point3D)(p.get(Integer.parseInt(k.nextToken())-1));
                ...
                public Point3D getPoint()
		{
			return point;
                 ...
                 */
                public int setDepth() {
			return super.depth = (int)(point.getz() * radix_percision_constant);
		}
                 ...
                 */
                public Shape3D cloneto(Point3D [] oldpoints, ArrayList newpoints) {
			return new Circle3D(correspondingpoint(point,oldpoints,newpoints),size,getColor());
		}
			...
			int s = (int)(2*size);
			applyColor(g);
			g.drawOval((int)(point.getx()-size),(int)(point.gety()-size),s,s);
		}
	}
        ...
        public class Sphere3D extends Circle3D
	{
        ...
        @Override
                public Shape3D cloneto(Point3D [] oldpoints, ArrayList newpoints) {
			return new Sphere3D(correspondingpoint(point,oldpoints,newpoints),size,getColor());
		}
                ...
                public void draw(Graphics g, BufferedImage image_buffer)
		{
			int s = (int)(2*size);
			applyColor(g);
			g.fillOval((int)(point.getx()-size),(int)(point.gety()-size),s,s);
		}
	}
                ...
                public Line3D(StringTokenizer k, ArrayList p)
		{
			...
			if (k.hasMoreTokens())
			{
                 ...
                 */
                public int setDepth() {
			return super.depth = (int)((point1.getz()+point2.getz())/2.0 * radix_percision_constant);
		}
			...
			return new Line3D(
                ...
                public void draw(Graphics g, BufferedImage image_buffer)
		{
			//Sxystem.out.println("line "+(int)point.getx()+" "+(int)point.gety()+" "+(int)image.getx()+" "+(int)image.gety());
			applyColor(g);
        ...
        public class ThickLine3D extends Line3D
	{
                ...
                public void draw(Graphics g, BufferedImage image_buffer)
		{
			...
			for (int i = n; i <= halfway; i++)
					g.drawLine((int)point1.getx()-n,(int)point1.gety()-i,(int)point2.getx()-n,(int)point2.gety()-i);
			for (int i = n; i <= halfway; i++)
					g.drawLine((int)point1.getx()-i,(int)point1.gety()-n,(int)point2.getx()-i,(int)point2.gety()-n);
			for (int i = n; i <= halfway; i++)
					g.drawLine((int)point1.getx()-halfway,(int)point1.gety()-i,(int)point2.getx()-halfway,(int)point2.gety()-i);
			for (int i = n; i <= halfway; i++)
					g.drawLine((int)point1.getx()-i,(int)point1.gety()-halfway,(int)point2.getx()-i,(int)point2.gety()-halfway);
		}
	}
        ...
        public class Outline3D extends Generic3D implements Shape3D
	{
                ...
                public Outline3D(StringTokenizer k, ArrayList l)
		{
			...
			{	
				...
				arr.add(new Integer(s));
			}	
			p = new Point3D [arr.size()];
			
			for (int i = 0; i < p.length; i++)
				p[i] = (Point3D)(l.get(((Integer)(arr.get(i))).intValue()-1));
		}
        ...
        public class Cylinder3D extends Line3D
	{
		...
		{
			applyColor(g);
			int i   = (int)((Math.atan2(point2.gety()-point1.gety(),point2.getx()-point1.getx())+piovertwo)*radius+perimeter.length);
			...
			{	int 
				...
				p1 = (int)(perimeter[index1][0]+point1.getx());
				p2 = (int)(perimeter[index1][1]+point1.gety());
				p3 = (int)(perimeter[index2][0]+point2.getx());
				p4 = (int)(perimeter[index2][1]+point2.gety());
                ...
                public void draw(Graphics g, BufferedImage image_buffer)
		{
			...
			int i1   = (int)((Math.atan2(point2.gety()-point1.gety(),point2.getx()-point1.getx())+piovertwo)*radius+perimeter.length);
			int i2   = (int)((Math.atan2(point2.gety()-point1.gety(),point2.getx()-point1.getx())+piovertwo)*radius2+perimeter2.length);
			
			int 
			index1 =  i1 % perimeter .length,
			index2 = (i2 + perimeter2.length) % perimeter2.length;
			g.drawLine(	(int)(perimeter[index1][0] + point1.getx()),
						(int)(perimeter[index1][1] + point1.gety()),
						(int)(perimeter2[index2][0]+ point2.getx()),
						(int)(perimeter2[index2][1]+ point2.gety()));
                ...
                public void draw(Graphics g, BufferedImage image_buffer)
		{
			...
			g.drawLine(	(int)(perimeter[index1][0]+point1.getx()),
						(int)(perimeter[index1][1]+point1.gety()),
						(int)(perimeter[index2][0]+point2.getx()),
						(int)(perimeter[index2][1]+point2.gety()));
			i++;
			j--;
			...
			{	 
				...
				index2 = j%perimeter.length;
				g.drawLine(p1,p2,p3,p4);
				image_buffer.setRGB((int)(perimeter[index1][0]+point1.getx()),(int)(perimeter[index1][1]+point1.gety()),color);
				image_buffer.setRGB((int)(perimeter[index2][0]+point2.getx()),(int)(perimeter[index2][1]+point2.gety()),color);
				i++;
				j--;
			}
	...
	Object3D(){}
		...
		for (int i = 0; i < shape.length; i++)
		{
			try 
			{
				...
				//to store midpoints that will define point new outline based on interpolation
				...
				//create point new outline3d by interpolation
			...
			{//the shape is not point valid outline or form 3d
			//	e.printStackTrace();
		...
		for (int i = 0; i < point.length; i++) 
		{
			...
			if (lines[i].size() > 2) 
			{
				...
				//store all the midpoints (to become points of an outline) an point new array
				Point3D [] p = new Point3D[linemidpoints[i].size()];
				for (int j = 0; j < linemidpoints[i].size(); j++) 
					p[j] = (Point3D)(linemidpoints[i].get(j));
				
				//create point new array that will hold the outline points in correct order
				...
				while (destinationindex < p.length-1) 
				{
					...
					int shapeindex = 0;
					//look for point match in the shapes
