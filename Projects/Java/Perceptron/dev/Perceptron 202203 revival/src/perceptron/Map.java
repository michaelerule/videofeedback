...
public class Map {
    ...
    public synchronized void operate() {
        ...
        // We have drawn to the "buf" data buffer
    ...
    void render(int start, int end, int cx, int cy, complex r) {
        ...
        // Map values are stored in units of pixels in 8-bit fixed point. 
        // The constant offsets cx and cy are also in 8-bit fixed point, and
        // the "get" functions expect this format. We use Gauss's method for
        // multiplying complex numbers to rotate the map. These are precomputed
        // 8-bit fixed point constants for that. 
        int RX = (int)(r.real*256+0.5),
            RY = (int)(r.imag*256+0.5),
            RC = (int)((r.real+r.imag)*256+0.5);
        
        // Linear Congruential RNG seeded with stronger RNG at start of frame
        ...
        for (int i = start; i<end; i++) {
            ...
            //fx = real*RX - imag*RY + 127 >> 8;
            //fy = real*RY + imag*RX + 127 >> 8;
            // Gauss' method avoids one multiplication
            int x4 = imag * RX;
            int x5 = real * RY;
            fx = cx + (x5 + x4 + 127>>8);
            fy = cy + (x5 - x4 + RC*(imag-real) +127 >>8);
            int c;
            if (bound_op.test(fx,fy,i) == invert_bound) {
                // out of bounds (inless boundary test is inverted)
                c = outop.go(fx,fy,i);
                c = grad_op.go(i,c);
            } else {
            ...
            c ^= color_mask;  
            if (noise_level>0) c = blend((rn=rn*rA+rC)>>8, c, noise_level);
    ...
    public class Cache {
        public final TranslucetLayer translucency;
        public final MapCache        map_cache;
        public Cache() {
            translucency = new TranslucetLayer();
            map_cache    = new MapCache();
        }
        /**
         * Merge translucent effects into a single layer.
         */
        public class TranslucetLayer {
            ...
            // "synchronized" protects (C,A,off) from concurrent modification. 
            public synchronized void cache() {
                ...
                for (int i=0; i<MLEN; i++) {  // Loop over pixels
                    ...
                    if (g>0) {                // Add gradient if it is on
                        ...
                        int c = g==1? c1 : blend(c2,c1,w);
                        RGB.c24.composePremultiplied(compose, c, w);
                    }                         // Apply tint if it is on
                    if (t>0) RGB.c24.composePremultiplied(compose, q, t);
                    C[i] = compose[0];        // Save
                    A[i] = (char)compose[1];
                }
            }
        ...
        public class MapCache {
            ...
            public synchronized void cache() {
                ...
                for (int i=0; i<MLEN; i++) {
                    ...
                    int x4 = imag * RX;
                    int x5 = real * RY;
                    fxy[j  ] = cx + (x5 + x4 + 127>>8);
                    fxy[j|1] = cy + (x5 - x4 + RC*(imag-real) +127 >>8);
                }
            }
        }
    }
    ...
    public final GradOp[] grad_modes = {
        ...
        new GradOp("1-Color"){int go(int i,int c){return blend(c,gc1,w(i));}},
        new GradOp("2-Color"){int go(int i,int c){int g=w(i); return blend(c,blend(gc2,gc1,g),g);}}};
