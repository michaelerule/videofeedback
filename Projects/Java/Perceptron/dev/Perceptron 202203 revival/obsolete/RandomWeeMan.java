...
public class RandomWeeMan //extends JPanel
{
    ...
    public void paintComponent( Graphics g )
    {
        for( int i = 0 ; i < 4 ; i++ )
        {
            points1[i] = new Point2D.Double( ( Math.random() * .25 +  (double) i / 4 ) * frameWidth ,
                    ( Math.random() * .5 + .5 ) * frameHeight );
        }
        points1[4] = new Point2D.Double( ( Math.random() * .5 + .25 ) * frameWidth ,
                ( Math.random() * .25 + .125 ) * frameHeight );
        ...
        Point2D.Double midpoint = new Point2D.Double( ( points[1].getX() + points[2].getX() ) / 2 ,
