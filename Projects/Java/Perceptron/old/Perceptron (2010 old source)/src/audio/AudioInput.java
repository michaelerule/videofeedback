...
public class AudioInput {
    ...
    public void increment_visualisation(int n) {
        ...
        int index = visualisations.indexOf(current_visualisation);
        if (index < 0) {
            ...
            current_visualisation = visualisations.get(index);
        }
    }
    ...
    void compute_histogram() {
        ...
        for (int i = 1; i < histogram.length / 2; i++) {
            ...
            float weight = histogram[i] / low_volume_average;
            int RGB = Color.HSBtoRGB(i % interval / interval, 1.f, weight);
            R += (0x000000FF & (RGB >> 16));
            G += (0x000000FF & (RGB >> 8));
            B += (0x000000FF & RGB);
        ...
        for (int i = histogram.length / 2; i < histogram.length; i++) {
            ...
            float weight = histogram[i] / high_volume_average;
            int RGB = Color.HSBtoRGB(i % interval / interval, 1.f, weight);
            R += (0x000000FF & (RGB >> 16));
            G += (0x000000FF & (RGB >> 8));
            B += (0x000000FF & RGB);
