...
public final class VisualCortex {
    ...
    public void setSigmaan(float f) {
        sigmaan = f;
        estimateKernelSize();
        form_kernel();
    }

    public void setAaniso(float f) {
        aaniso = f;
        form_kernel();
    }
    ...
    public void setAiOrient(float f) {
        ai_orient = f;
    }

    public void setAeOrient(float f) {
        ae_orient = f;
    }

    public void setSigmaeOrient(float f) {
        sigmae_orient = f;
        estimateKernelSize();
        form_kernel_orient();
    }

    public void setSigmaiOrient(float f) {
        sigmai_orient = f;
        estimateKernelSize();
        form_kernel_orient();
    }
    ...
    public float getSigmae() {
        return sigmae;
    }

    public float getSigmai() {
        return sigmai;
    }

    public float getSigmaan() {
        return sigmaan;
    }

    public float getAaniso() {
        return aaniso;
    }
    ...
    public float getAiOrient() {
        return ai_orient;
    }

    public float getAeOrient() {
        return ae_orient;
    }

    public float getSigmaeOrient() {
        return sigmae_orient;
    }

    public float getSigmaiOrient() {
        return sigmai_orient;
    }

    private void estimateKernelSize() {
    ...
    public BufferedImage activationmap = new BufferedImage(size, size, BufferedImage.TYPE_INT_RGB);
    public BufferedImage orientationmap = new BufferedImage(size, size, BufferedImage.TYPE_INT_RGB);
    ...
    float coupling3(int ki, int kj, int i2, int j2, int io, int jo) {
        int ct0 = cost[io][jo];
        int st0 = sint[io][jo];
        return tofloat(kani[ki][kj] * costwodtheta(ct0, st0, cost[i2][j2], sint[i2][j2]) * costwodtheta(ct0, st0, kcos[ki][kj], ksin[ki][kj]) >> 16);
    }
    ...
    public void step() {
         ...
         * stepRange(0,size/4,0,size); } }; Thread t2 = new Thread() { public
         * void run(){ stepRange(size/4,size/2,0,size); } }; Thread t3 = new
         * Thread() { public void run(){ stepRange(size/2,size/4*3,0,size); } };
         * Thread t4 = new Thread() { public void run(){
         * stepRange(size/4*3,size,0,size); } }; t1.start(); t2.start();
         * t3.start(); t4.start();
         * 
         * try { t1.join(); } catch (InterruptedException ex) {
         * Logger.getLogger(VisualCortex.class.getName()).log(Level.SEVERE,
         * null, ex); } try { t2.join(); } catch (InterruptedException ex) {
         * Logger.getLogger(VisualCortex.class.getName()).log(Level.SEVERE,
         * null, ex); } try { t3.join(); } catch (InterruptedException ex) {
         * Logger.getLogger(VisualCortex.class.getName()).log(Level.SEVERE,
         * null, ex); } try { t4.join(); } catch (InterruptedException ex) {
         * Logger.getLogger(VisualCortex.class.getName()).log(Level.SEVERE,
         * null, ex); }
         */
