...
public class CellModel {
    ...
    public class Cell {
        ...
        short X;
        short Y;
        protected float[] rx = new float[SMOOTH_WALK], ry = new float[SMOOTH_WALK];
        protected float[] dx = new float[TRAILS + 1], dy = new float[TRAILS + 1];
        float size;
        boolean dividing;
        Image image;
        ...
        public Cell(float nx, float ny, float h, float s) {
            ...
            ID = NUM_ID++;
            if (NUM_ID == 0) {
                System.out.println("WARNING: Cell ID overflow");
            }
            dividing = false;
            image = trailer;
        }
        ...
        public void advance_location() {
            if (!dividing) {
                ...
                /*
                 * if (Dx >= W) Dx = W - 1 ; else if (Dx < 0) Dx = 0 ; if (Dy >=
                 * H) Dy = H - 1 ; else if (Dy < 0) Dy = 0 ;
                 */
                rx[0] = Dx;
                ry[0] = Dy;
            }
            ...
            if (dividing) {
                ...
                if (deltax * deltax + deltay * deltay < 2 * AREA) {
                    ...
                    size -= nsize;
                    cells.add(new Cell(dx[TRAILS], dy[TRAILS], HUE + .05553643f, nsize));
                    setHue(HUE - .0322560f);
                    dividing = false;
                    image = trailer;
                }
    ...
    public void resetGraphics() {
        if (antialiased) {
            display_graphics = ColorUtility.fancy(display.createGraphics());
        } else {
            display_graphics = display.getGraphics();
        }
    }
    ...
    public void seedUniformRandom(int n) {
        for (int i = 0; i < n; i++) {
            addCell(rand.nextFloat() * W, rand.nextFloat() * H);
        }
    }
    ...
    public CellDrawer[] drawers = {new CellDrawer("Polygon") {
        @Override
        public void draw(Cell c, int color) {
            float v = 0;
            for (int i = 0; i < TRAILS; i++) {
                float W = (float) i / TRAILS;// Math.pow(.8,10 - 10. * i /
                // TRAILS);
                W = W * (1 - W);
