    ...
    public void step() {
        for (int i = 0; i < H; i++) {
            for (int j = 0; j < W; j++) {
                ...
                Vprime = Vprime * .9f + .1f * V;
                if (V > VMAX - 1) {
                    Vprime = 0;
                } else if (V < VMIN) {
                    Vprime = VMAX;
                }
                cell[i][j].V = V + (float) rand.nextGaussian() * .5f;
                cell[i][j].Vprime = Vprime;

                // int color1 = (int)(0xFF*((V-VMIN)/(VMAX-VMIN))) & 0xFF ;
                /*
                * data.setElem((i<<SIZE)+j, ColorUtility.average(
                * rand.nextInt(0x1000000) , 26 , ColorUtility.average(COLORA,
                * color1, COLORB, 256 - color1 ) , 230 ) );
                */

                float h = ((V - VMIN) / (VMAX - VMIN));
                data.setElem((i << SIZE) + j, Color.HSBtoRGB(h, 1f, 1f));
