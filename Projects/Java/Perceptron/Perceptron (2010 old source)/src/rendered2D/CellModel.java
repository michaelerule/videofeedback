...
public class CellModel {
    ...
    public synchronized void setAntiAliased(boolean antialias) {
        antialiased = antialias;
        resetGraphics();
    }
    public synchronized void setArea(float f) {
        AREA = f;
    }
    public synchronized void setGrowth(float pow) {
        growthRate = pow;
    }
    public synchronized void setRate(float f) {
        moveRate = f;
    }
    public synchronized void setSigma(float f) {
        searchRate = f;
    }
    ...
    public class Cell {
        ...
        short X;
        short Y;
        protected float[] rx = new float[SMOOTH_WALK],  ry =
          new float[SMOOTH_WALK];
        protected float[] dx = new float[TRAILS + 1],  dy = new float[TRAILS +
          1];
        float size;
        boolean dividing;
        Image image;
        ...
        synchronized void setHue(float h) {
            HUE = h;
            //color = new Color(Color.HSBtoRGB(h, 1.f, 1.f));
            color = Color.HSBtoRGB(h, 1.f, 1.f);
        }
        public synchronized void bound() {
            for (int i = 0; i < SMOOTH_WALK; i++) {
                if (rx[i] >= W) rx[i] = W - 1;
        ...
        public synchronized void advance_location() {
            if (!dividing) {
    ...
    public synchronized void resetCellArray() {
        cells.clear();
        this.NUM_ID = 0;
    }
