...
public class Tree3D1 extends Object3D 
{
	...
	private class Branch {
		...
		{		
			...
			{branch1, branch2, connect1, connect2, top[0], top[1], top[2], top[3]};	
			...
			{		
				...
				rotate[i].y = y;
				rotate[i].z = z;
				x = cosgamma * rotate[i].x + singamma * rotate[i].y;
				y = cosgamma * rotate[i].y - singamma * rotate[i].x;
				z = rotate[i].z;
				rotate[i].x = x;
				rotate[i].y = y;
				rotate[i].z = z;
				x = cosdelta * rotate[i].x + sindelta * rotate[i].z;
				y = rotate[i].y;
				z = cosdelta * rotate[i].z - sindelta * rotate[i].x;
				rotate[i].x = x;
				rotate[i].y = y;
				rotate[i].z = z;
			}
			...
			RotateablePoint3D [] branch1base = {
				top[0], top[1], connect1, connect2
			};
			RotateablePoint3D [] branch2base = {
				top[2], top[3], connect2, connect1
			};
		...
		public Branch(Branch stem, double r, double alpha, double beta, double w, boolean wireframe,
		ArrayList points, ArrayList rotateablepoints, ArrayList masterpoints, ArrayList shapes)
		{
	...
	public Tree3D1() {
		...
		for (int iter = 0; iter < 9; iter++)
		{
			...
			//	double w = a1[i].width/1.414;
				...
				Branch [] newbranches = a1[i].branch(1,d,.7,.7,false,true,false,points,rotateablepoints,masterpoints,lines);
				a2[counter++] = newbranches[0];
				a2[counter++] = newbranches[1];	
			}
			a1 = a2;
		...
		for (int i = 0; i < a1.length; i++) {
			a1[i].cap(points,rotateablepoints,masterpoints,lines,true,false,false,false);
...
/*	
	public class TaperedStem3D extends Cylinder3D {
		...
		public TaperedStem3D(Point3D a, Point3D b, double r) 
		{
			super(a,b,r);
		}
		...
		public void draw(Graphics g, BufferedImage image_buffer) 
		{
			...
			index2 = (i + perimeter.length)%perimeter.length;
			...
			index1 = (i + perimeter.length/2 - 1)%perimeter.length;
			index2 = (i + perimeter.length/2 + 1)%perimeter.length;
			g.drawLine(	(int)(perimeter[index1][0]+a.getx()),
						(int)(perimeter[index1][1]+a.gety()),
						(int)(perimeter[index2][0]+b.getx()),
						(int)(perimeter[index2][1]+b.gety()));
