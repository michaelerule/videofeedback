...
public class Kernel1551883364867
{
 ...
 public static void prepare(){
  out = new BufferedImage(1024,1024,BufferedImage.TYPE_INT_RGB);
  disp = new BufferedImage(1024,1024,BufferedImage.TYPE_INT_RGB);
  din = new int[(1024*1024)];
  dtmp = new int[(1024*1024)];
  dout = new int[(1024*1024)];
 }
 ...
 public static void convolveStage1(int threadnumber, int nthreads)
 {
  ...
  int Cstart = 1024/nthreads*threadnumber;
  int Cstop = threadnumber==nthreads-1? 1024: 1024/nthreads*(threadnumber+1);
  for (int C=Cstart; C<Cstop; C++){
   next = bufferOut.getElem(C+1024) & 0xfefefe;
   curr = bufferOut.getElem(C ) & 0xfefefe;
   bufferTmp[C] = pair = prev = next+curr>>1 & 0xfefefe;
   for (int R=1; R<(1024 -1); R++){
    curr = next; next = bufferOut.getElem(C+(R+1)*1024) & 0xfefefe;
    prev = pair; pair = next+curr>>1 & 0xfefefe;
    bufferTmp[C+R*1024]=prev+pair>>1;
   }
   bufferTmp[C+(1024 -1)*1024]=pair>>1;
  }
 }
 ...
 public static void convolveStage2(int threadnumber, int nthreads)
 {
  ...
  int[] bufferTmp = dtmp;
  DataBuffer bufferOut = out.getRaster().getDataBuffer();
  int Rstart = 1024/nthreads*threadnumber;
  int Rstop = threadnumber==nthreads-1? 1024: 1024/nthreads*(threadnumber+1);
  int Istart = Rstart*1024;
  int Istop = Rstop*1024;
  if (0!=BLUR||0!=SHARPEN){
   int curr, next, prev, pair;
   for (int R=Rstart; R<Rstop; R++){
    next = bufferTmp[R*1024 +1] & 0xfefefe;
    curr = bufferTmp[R*1024 ] & 0xfefefe;
    bufferIn[R*1024] = pair = prev = next+curr>>1 & 0xfefefe;
    for (int C=1; C<(1024 -1); C++){
     curr = next; next = bufferTmp[R*1024 +C+1] & 0xfefefe;
     prev = pair; pair = next+curr>>1 & 0xfefefe;
     bufferIn[R*1024 +C]= prev+pair>>1;
    }
    bufferIn[R*1024 +(1024 -1)]=pair;
   }
...
int rstart = 1024/nthreads*thn;
int rstop = thn==nthreads-1? 1024: 1024/nthreads*(thn+1);
int cstart = 0;
int cstop = 1024;
int halfway = 1024/2;
switch(DUPLICATION) {
 case 1:
  rstart = 1024/nthreads*thn;
  rstop = thn==nthreads-1? 1024: 1024/nthreads*(thn+1);
  cstart = 0;
  cstop = 1024/2;
 break;
 case 2:
  rstart = 1024/2/nthreads*thn;
  rstop = thn==nthreads-1? 1024/2+1: (1024/2+1)/nthreads*(thn+1);
  cstart = 0;
  cstop = 1024;
 break;
 case 3:
...
{
 case 0:
     while (x<(((1024 -3)*256)*2)) x+=(((1024 -3)*256)*2);
     while (y<(((1024 -3)*256)*2)) y+=(((1024 -3)*256)*2);
     x=(x/((1024 -3)*256)&1)==1?x%((1024 -3)*256):(((1024 -3)*256)-1)-x%((1024 -3)*256);
     y=(y/((1024 -3)*256)&1)==1?y%((1024 -3)*256):(((1024 -3)*256)-1)-y%((1024 -3)*256);
     break;
 case 1:
     while (x<((1024 -3)*256))x+=((1024 -3)*256);
     while (y<((1024 -3)*256))y+=((1024 -3)*256);
  x%=((1024 -3)*256);
  y%=((1024 -3)*256);
  break;
 case 2:
  x = x<0?0:x>((1024 -3)*256)?((1024 -3)*256):x;
  y = y<0?0:y>((1024 -3)*256)?((1024 -3)*256):y;
  break;
 case 3:
 case 4:
}
...
switch(GRADIENT) {
 case 0:
  break;
 case 1: {
  int alpha = col*256/1024*GRADIENT_GAIN+GRADIENT_BIAS*256>>8;
  ...
  break;
 case 2: {
  int alpha = row*256/1024*GRADIENT_GAIN+GRADIENT_BIAS*256>>8;
  alpha = alpha<0?0:alpha>0xff?0xff:alpha;
  c = ((alpha*(0&0xff00ff)+(0x100 -alpha)*(c&0xff00ff))+0x800080>>8&0xff00ff|(alpha*(0&0x00ff00)+(0x100 -alpha)*(c&0x00ff00))+0x008000>>8&0x00ff00);
  }
  break;
 case 3: {
  int alpha = ((row-1024/2)*256/1024*GRADIENT_BIAS+(col-1024/2)*256/1024*GRADIENT_GAIN>>7);
  alpha = alpha*alpha>>8;
  alpha = alpha<0?0:alpha>0xff?0xff:alpha;
  c = ((alpha*(0&0xff00ff)+(0x100 -alpha)*(c&0xff00ff))+0x800080>>8&0xff00ff|(alpha*(0&0x00ff00)+(0x100 -alpha)*(c&0x00ff00))+0x008000>>8&0x00ff00);
  }
  break;
 case 4: {
  int alpha = (row-1024/2)*512/1024*GRADIENT_BIAS >> 6;
  int beta = (col-1024 /2)*512/1024 *GRADIENT_GAIN >> 6;
  ...
  break;
 case 5: {
  if ((((row-1024/2)*512/1024)^(((row-1024/2)*512/1024)>>31))>((GRADIENT_BIAS)^((GRADIENT_BIAS)>>31))) c=0;
  if ((((col-1024 /2)*512/1024)^(((col-1024 /2)*512/1024)>>31))>((GRADIENT_GAIN)^((GRADIENT_GAIN)>>31))) c=0;
  }
  break;
 default:
  break;
...
switch(DUPLICATION) {
 ...
 case 1:
  bufferOut.setElem(i,c);
  bufferOut.setElem(row*1024 +1024 -1-col,c);
 break;
 case 2:
  bufferOut.setElem(i,c);
  if (row>0)
  bufferOut.setElem((1024 -row)*1024 +1024 -1-col,c);
 break;
 case 3:
  bufferOut.setElem(i,c);
  int ty1 = (1024 -1)*1024 -row*1024;
  int tx1 = 1024 -1-col;
  bufferOut.setElem(row*1024 +tx1,c);
  bufferOut.setElem(ty1+tx1,c);
  bufferOut.setElem(ty1+col,c);
 break;
 case 4:
  c = (c>>1)&0x7f7f7f;
  int tx = row*1024 +1024 -1-col;
