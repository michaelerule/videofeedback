...
public class KERNELVERSION 
{
	...
	public static void render(int threadnumber, int nthreads)
	{
		...
		float huerotate = Recurrent.HUE*0.01953125f;
		...
		for (int i=istart; i<istop; i++)
		{
			...
			if (HSV_OPERATOR==1)
			{
				...
				float max      = r>b?g>r?g:r:g>b?g:b; // also maximum
				r = (r - max)*resaturate + max;
				g = (g - max)*resaturate + max;
				b = (b - max)*resaturate + max;
			
				//value-resaturate*value+min*resaturate = min2
				...
				{
					float hue;
					if      (value==r) hue = 6+(g-b)/chroma;
					else if (value==g) hue = 2+(b-r)/chroma;
					else               hue = 4+(r-g)/chroma;
					r=g=b=min;
					hue = (hue+huerotate)%6;
					switch((int)hue)
					{
						case 0: { r+=chroma         ; g+=chroma*(hue)   ;                   ;} break;// red to yellow
						case 1: { r+=chroma*(2-hue) ; g+=chroma         ;                   ;} break;// yellow to green
						case 2: {                   ; g+=chroma         ; b+=chroma*(hue-2) ;} break;// green to cyan
						case 3: {                   ; g+=chroma*(4-hue) ; b+=chroma         ;} break;// cyan to blue
						case 4: { r+=chroma*(hue-4) ;                   ; b+=chroma         ;} break;// blue to magenta
						case 5: { r+=chroma         ;                   ; b+=chroma*(6-hue) ;} break;// magenta to red
					}
				}
