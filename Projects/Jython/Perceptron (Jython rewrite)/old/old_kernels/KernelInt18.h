...
public class KERNELVERSION 
{
	...
	public static void render(int threadnumber, int nthreads)
	{
		...
		float resaturate = Recurrent.SATURATION*0.0078125f;
		float rBrightness = resaturate*(Homeostatic.BRIGHTNESS*256-RMEAN*RVAR) / 256.f;
		float gBrightness = resaturate*(Homeostatic.BRIGHTNESS*256-GMEAN*GVAR) / 256.f;
		float bBrightness = resaturate*(Homeostatic.BRIGHTNESS*256-BMEAN*BVAR) / 256.f;
		float rContrast   = resaturate*RVAR/256.f;
		float gContrast   = resaturate*GVAR/256.f;
		float bContrast   = resaturate*BVAR/256.f;
		...
		for (int i=istart; i<istop; i++)
		{
			...
			// scaling value amounts to a contrast adjustment and so 
			// is not represented here.
			// Scaling saturation can be done quickly without knowledge
			// of the hue -- it is a contrast adjustment relative to the
			// brightest component in the color
			float max = (r>b?g>r?g:r:g>b?g:b)*(1-resaturate); // saturation adjustment
			r = r*rContrast+rBrightness+max;
			g = g*gContrast+gBrightness+max;
			b = b*bContrast+bBrightness+max;
			...
			if (HSV_OPERATOR==1)
			{
				...
				if (chroma!=0) //with no saturation, hue is undefined
				{
					...
					switch(hue*6>>11)
					{
						case 0: { R+=chroma                 ; G+=chroma*(hue)*768>>18      ;                            ;} break;// red to yellow
						case 1: { R+=chroma*(2*341-hue)*768>>18; G+=chroma                 ;                            ;} break;// yellow to green
						case 2: {                           ; G+=chroma                 ; B+=chroma*(hue-2*341)*768>>18 ;} break;// green to cyan
						case 3: {                           ; G+=chroma*(4*341-hue)*768>>18; B+=chroma                  ;} break;// cyan to blue
						case 4: { R+=chroma*(hue-4*341)*768>>18;                           ; B+=chroma                  ;} break;// blue to magenta
						case 5: { R+=chroma                 ;                           ; B+=chroma*(6*341-hue)*768>>18 ;} break;// magenta to red
					}
				}
			}
			c = PACKRGB32(R,G,B);
