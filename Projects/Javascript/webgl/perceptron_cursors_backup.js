function sprite_renderer(gl,npoint,ndots,canvas,alpha) {
    
    // Total sprites used:
    // all cursors (npoint)
    // Times number of trailing "dots" (ndots)
    // Plus one point for a virtual mouse cursor
    var ndots_total = npoint*ndots+1;
    ...
    var offsets = new Float32Array(2*ndots_total);
    ...
    for (var i=npoint; i<ndots_total; i++) {
        offsets[2*i]=spritecol;
        offsets[2*i+1]=spriterow;
    }
    // For .. reasons .. we need to reverse this array
    // except the last sprite which needs to stay at the end
    var newoffsets = new Float32Array(2*ndots_total);
    for (var i=0; i<ndots_total; i++) {
        newoffsets[i*2] = offsets[(ndots_total-2-i)*2];
        newoffsets[i*2+1] = offsets[(ndots_total-2-i)*2+1];
    }
    var spritecol = ((NSPRITE+1|0)%SPRITE_ATLAS_COLS)|0;
    var spriterow = ((NSPRITE+1)/SPRITE_ATLAS_COLS)|0;
    newoffsets[(ndots_total-1)*2] = spritecol;
    newoffsets[(ndots_total-1)*2+1] = spriterow;
    offsets=newoffsets;
    //console.log(offsets);
    ...
    var fragment = `
        ...
        void main() {
            ...
            if (rr>0.2 && rr<0.48 && v.y>1.5) {
                alpha=1.0;
                frgnd=vec4(sign(cos(rr*20.0))*0.5+0.5);
            }
    ...
    function render_sprites(background,sprites,output,perceptron_model) {
        ...
        */
        ...
        gl.drawArrays(gl.POINTS, 0, ndots_total); check_error(gl);
